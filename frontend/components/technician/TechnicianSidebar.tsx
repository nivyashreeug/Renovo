...
export default function TechnicianSidebar({ mobileOpen = false, onClose }: TechnicianSidebarProps) {
  ...
  return (
    <>
      {mobileOpen && (
        <button
          type="button"
          aria-label="Close technician sidebar"
          onClick={onClose}
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex h-screen flex-col border-r border-white/10 bg-[#050816]/95 px-3 py-4 text-sm text-slate-200 backdrop-blur-2xl transition-transform duration-300 lg:sticky lg:top-0 lg:z-auto lg:translate-x-0",
      ...
      >
          ...
          <div className="flex items-center gap-2">
            {onClose && (
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl border border-white/10 bg-white/5 p-2 text-white/70 transition hover:border-[#00F5FF]/30 hover:text-white lg:hidden"
            ...
            <button
              ...
              className="rounded-xl border border-white/10 bg-white/5 p-2 text-white/70 transition hover:border-[#00F5FF]/30 hover:text-white"
            >
              <Menu size={16} />
            </button>
          </div>
        ...
        <div className="space-y-3 pt-3">
          <button
            type="button"
            onClick={() => {
              router.push("/dashboard/technician/settings")
              onClose?.()
            }}
            className={`w-full rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-left text-white/70 transition hover:border-[#00F5FF]/30 hover:text-white ${collapsed ? "flex items-center justify-center" : ""}`}
          >
            <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={logout}
            className={`w-full rounded-2xl border border-[#FF4D6D]/20 bg-[#FF4D6D]/10 px-3 py-3 text-left text-white/80 transition hover:bg-[#FF4D6D]/15 ${collapsed ? "flex items-center justify-center" : ""}`}
          >
            <div className="flex items-center gap-3">
              <LogOut size={16} />
